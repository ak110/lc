...
`DummyForm.GetVisibleOwner()` はowner候補としてMainForm / ButtonLauncherForm / `Form.ActiveForm` の順で返す。
`Form.ActiveForm` フォールバックは、MainFormが非表示でConfigForm等のモーダルダイアログが開いている最中に
スケジューラーが発火したケースをカバーする。この場合、NotificationFormはモーダルダイアログをownerとして
`ShowDialog` するため、閉じたときはWinFormsの標準挙動でモーダルダイアログへフォーカスが戻る。

一方、launcher内に表示フォームが一切無い状態（`HideFirst=true`での起動直後やMainForm hide後）で
