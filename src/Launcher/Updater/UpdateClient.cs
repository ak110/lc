...
public class DownloadUpdatesEventArgs : EventArgs {
	UpdateFile[] files;
	public DownloadUpdatesEventArgs(UpdateFile[] f) {
		files = f;
	}
	/// <summary>
	/// ファイル情報のリスト
...
public class UpdateClient {
	...
	List<UpdateFile> downloadFiles = null;
	string tempDir;
	string batchFileName;

	public UpdateClient(UpdateConfig config, UpdateRecord record) {
		this.config = config;
		this.record = record;
	}
	...
	public UpdateInfo GetUpdateInfo(string url) {
		...
		info = UpdateInfo.DeserializeFromString(Encoding.UTF8.GetString(data));
	...
	public void DoUpdate() {
		...
		try {
			...
			if (!toBeContinue) {
				System.Diagnostics.Debug.Fail("なんか変なところに来た(´・ω・`)");
				Directory.Delete(tempDir, true);
			}
		}
	...
	void ListupDownloadFiles() {
		...
		// リストアップ
		downloadFiles = new List<UpdateFile>();
		foreach (UpdateFile f in info.Files) {
			FileRecord r = FindFileRecord(f);
			if (r == null || f.Hash != r.Hash) {
				// エントリが存在しないか、ハッシュが異なっているならダウンロードすべき。
				downloadFiles.Add(f);
			}
		}

		// ダウンロードすべきファイルがないならコールバック
	...
	private void InnerDownloadUpdates() {
		...
		// ダウンロード
		foreach (UpdateFile file in downloadFiles) {
			...
			// zipなら解凍、でなければそのまま書き出す。
			if (Path.GetExtension(extractPath).ToLower() == ".zip") {
				Directory.CreateDirectory(extractPath);
				using (ZipReader zip = new ZipReader(data)) {
		...
		foreach (UpdateEntry entry in info.Entries) {
			if (entry.Obsoleted) {
	...
	/// <summary>
	/// UpdateEntryから対象ファイル名を作成
	/// </summary>
	private static string GetDestinationFileName(UpdateEntry entry) {
	...
	/// UpdateEntryから対象ファイル名を作成
	/// </summary>
	private static string GetDestinationFileName(string dir, UpdateEntry entry) {
	...
	/// <summary>
	/// UpdateFileに対応するFileRecordを検索。無ければnull
	/// </summary>
	private FileRecord FindFileRecord(UpdateFile f) {
		return record.FileRecords.Find(delegate(FileRecord fr) {
	...
	private void DoUpdateRecord() {
		...
		record.OldNotices = info.Notices.ConvertAll(n => (Notice)n.Clone());
				...
				while (true) {
					...
					if (DownloadProgress != null) {
						DownloadProgress(this, new DownloadProgressEventArgs(relUrl,
							...
							(n == buffer.Length ? 0 : (int)memory.Position)));
					}
				}
				// ダウンロード完了コールバック
