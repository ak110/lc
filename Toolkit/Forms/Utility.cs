...
namespace Toolkit.Forms {
	/// <summary>
	/// System.Windows.Forms関係のヘルパ関数など。
	/// </summary>
	public static class Utility {
		/// <summary>
		/// クリッピングしつつフォームの位置をセット
		/// </summary>
		...
		public static void ActivateForce(Form form) {
			using (Toolkit.Windows.AttachThreadInput ati = new Toolkit.Windows.AttachThreadInput()) {
				int time = 0;
				try {
		...
		/// ノードを再帰的に探す。
		/// </summary>
		/// <param name="tree">検索するツリービュー</param>
		/// <param name="predict">検索条件</param>
		...
		/// ノードを再帰的に探す。
		/// </summary>
		/// <param name="node">検索する根ノード</param>
		/// <param name="predict">検索条件</param>
		/// <returns>見つからなければnull</returns>
		...
		/// アイテムを探す。
		/// </summary>
		/// <param name="node">検索するリストビュー</param>
		/// <param name="predict">検索条件</param>
		/// <returns>見つからなければnull</returns>
		...
		#region ドラッグ＆ドロップ実装用ヘルパ

		/// <summary>
		/// DragEnter, DragOverでe.Effectを設定する処理。
		/// ctrlキー押されてて AllowedEffect に Copy フラグがあればコピー、
		/// shiftキー押されてて AllowedEffect に Move フラグがあれば移動、
		/// AllowedEffect に defaultEffect があればそれ、
		/// 以上に該当しなければ None。
		/// </summary>
		public static void SetDropEffect(DragEventArgs e, DragDropEffects defaultEffect) {
			if ((e.AllowedEffect & DragDropEffects.Copy) != 0 && (e.KeyState & 0x8) != 0) { // ctrlキー
				e.Effect = DragDropEffects.Copy;
			} else if ((e.AllowedEffect & DragDropEffects.Move) != 0 && (e.KeyState & 0x4) != 0) { // shiftキー
				e.Effect = DragDropEffects.Move;
			} else if ((e.AllowedEffect & defaultEffect) != 0) { // デフォルト
		...
		/// <param name="chromosome">コントロール</param>
